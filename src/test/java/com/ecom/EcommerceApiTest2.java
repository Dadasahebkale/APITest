...
import com.ecom.pojo.LoginRequest;
import com.ecom.pojo.LoginResponse;
import com.ecom.pojo.OrderDetail;
import com.ecom.pojo.Orders;
import com.ecom.pojo.ProductResponse;
...
public class EcommerceApiTest2 {
	...
	public static void main(String[] args) {
		...
		loginRequest.setUserEmail("dk@abc.com");
		loginRequest.setUserPassword("Ganesh@01");
		...
		RequestSpecification reqAddProduct = given().log().all().spec(addProductBaseReq).param("productName", "Laptop")
				.param("prodctAddeBy", userId).param("productCategory", "fashion").param("productSubCategory", "shirts")
				.param("productPrice", "12000").param("productDescription", "Lenova").param("productFor", "men")
				.multiPart("productImage", new File("C:\\Users\\Ganesh\\Postman\\files\\Laptop.png"));

		String addProdctResponse = reqAddProduct.when().post("/api/ecom/product/add-product").then().log().all()
				.extract().response().asString();

		JsonPath js = new JsonPath(addProdctResponse);
